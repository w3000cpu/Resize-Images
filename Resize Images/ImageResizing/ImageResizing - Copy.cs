...
public class ImageResizing
        ...
        try
        {
            if (!string.IsNullOrEmpty(saveFolderName))
            {
                ...
                if(i>64)
                {
                 ...
                 Resizing(ImagesFiles[i], format, saveFolderName, Images[i], Width[i], Height[i], Resolution[i]);
    ...
    private void Resizing(string imageFile, ImageFormat format, string saveFolderName, Image image, int width, int height, float resolution)
    {
        ...
        try
        {
            ...
            //WORKING
            //using (Bitmap original = (Bitmap)Image.FromFile(imageFile))
            //{
            //    using (Bitmap resized = new Bitmap(original, new Size(width, height)))
            //    {
            //        resized.SetResolution(resolution, resolution);
            //        resized.Save(saveFolderName + "\\" + imageFile.Split('\\').Last(), format);
            //    }
            //}
            ...
            {
                byte[] imageFileInBytes = Encoding.ASCII.GetBytes(imageFile);

                memStream.Write(imageFileInBytes, 0, imageFileInBytes.Length);
                using (Image image1 = Image.FromFile(imageFile))
                {
                    ...
                    using (var resized = new Bitmap(width, height)) {
                        using (var graphics = Graphics.FromImage(resized))
                        {
                            ...
                            using (var wrapMode = new ImageAttributes())
                            {
                                ...
                                graphics.DrawImage(image1, destRect, 0, 0, width, height, GraphicsUnit.Pixel, wrapMode);
                                graphics.Save();
                    ...
                    //image1.Save(saveFolderName + "\\" + imageFile.Split('\\').Last(), format);
